...
import org.iscas.iterativeBroadcastJoin.data.{DataGenerator, SkewedDataGenerator}
import org.iscas.iterativeBroadcastJoin.join._

object RunBenchmark2 extends App {
  /**
    * 计算代码块的执行时间
    * @param block
    * @tparam R
  ...
  def runTest(generator: DataGenerator,
    ...
    time{
      val out=joinType match{
        ...
        case _:IterativeBroadcastJoinType => IterativeBroadcastJoin2.join(
          spark,
          spark.read.load(generator.getLargeTableName),
          spark.read.load(generator.getMediumTableName)
        )
